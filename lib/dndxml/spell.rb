...
module DnDXML
	...
	module SpellClass
		...
		ALL = [BARD, CLERIC, DRUID, PALADIN, RANGER, SORCERER, WARLOCK, PALADIN]
	end
	...
	class Spell
		...
		class UnitType
			...
			def initialize(element)
				text = element.text
				num = text.to_i
				self.value = num.nil? ? text : num
				self.unit = element.attributes['unit']
			end
			
			def to_s
				return @value if @value.kind_of? String
				return "#{@value} #{@unit}" if @value == 1
				return "#{@value} #{self.pluralized_unit}"
			end
		...
		def initialize(xmlnode)
			...
			xmlnode.elements.each do |element|
				...
				when 'class'
					...
					raise InvalidXMLError.new "Invalid <class> value '#{value}'" unless SpellClass::ALL.contains? value
					@classes << value unless @classes.contains? value
				when 'castingtime'
		...
		def to_xml
			...
			@classes.each do |class|
				element.add_element('class').add_text(class.to_s)
			end
		...
		def subtitle
			...
			when 0
				...
				"#{LEVEL_NAMES[@level]} #{@school.to_s.capitalize}"
			end
		...
		def raise_unless_valid
			...
			raise InvalidXMLError.new "Invalid <school> value '#{@school}'" unless SpellSchool::ALL.contains? @school
